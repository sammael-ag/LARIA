export const faqArticles = [
  {
    ...
    content: `
      <div class="faq-article-content">
        <p><strong>Hľadáš únik z korporátneho hluku a politického tlaku?</strong></p>
        
        <p>Vitaj v priestore, ktorý bol vytvorený preto, aby ti vrátil suverenitu.
        <strong>LARIA</strong> nie je ďalšia sociálna sieť, ktorá ťa chce udržať priklincovaného k obrazovke. Je to tichý, nezničiteľný svietiaci kryštál na pozadí (<em>laria.space</em>). Prepojí ťa s ľuďmi a tvorcami v tvojom okolí, urobí svoju robotu, nič si nezapamätá – a nechá ťa žiť tvoj reálny život.</p>
        ...
        <ul>
          ...
          <li><strong>Nulová stopa:</strong> Aplikácia beží, spojí, ale nič neukladá na vzdialené servery. Zodpovednosť za dáta a tvoje SHA je plne v Tvojich rukách.</li>
          <li><strong>Tokenomika na Base:</strong> Hlavné používateľské rozhranie pre rastúci ekosystém s fixným supply (10 000 000 LARIA), krytý smart contractmi a v blízkej budúcnosti reálnou hodnotou.</li>
        </ul>
        ...
        <ul>
          ...
          <li><strong>Nová Aria AI:</strong> Odpútanie od ťažkopádneho Google backendu smerom k plne integrovanému a rýchlemu skutočnému asistentovi v Rust jadre.</li>
        </ul>
        <br/>
    ...
    content: `
      <div class="faq-article-content">
        ...
        <div class="faq-image-container">
          <img src="./images/panorama1.png" alt="Cesta vytvorenia vizitky v systéme LARIA" class="faq-image" onclick="window.openImageModal(this.src)" />
        ...
        <ol>
          ...
          <li><strong>Zálohovanie SHA odtlačku:</strong> V hlavnom zobrazení Ateliéru - Dashboardu sa klikom na sekciu "Nastavenia" otvorí ponuka, v ktorej je úplne navrchu ako prvá položka, políčko s Tvojim unikátnym sha odtlačkom. Ten je možné kopírovať klikom na ikonku, alebo odoslanie emailom - ak existuje email adresa v Tvojich uložených údajoch, vyplní a odošle sa automaticky, ak nie, môžeš ju zadať. <strong> Túto email adresu uloží LARIA do Tvojho úložiska v zariadení.</strong></li>
        </ol>
      </div>
    `
  },
  {
    ...
    content: `
      <div class="faq-article-content">
        ...
        <div class="faq-image-container">
          <img src="./images/panorama2.png" alt="Vyhľadanie a pridanie kontaktu" class="faq-image" onclick="window.openImageModal(this.src)" />
        </div>
        ...
        <div class="faq-image-container">
          <img src="./images/panorama3.png" alt="Obnova účtu v systéme LARIA" class="faq-image" onclick="window.openImageModal(this.src)" />
        </div>

        <ul>
          <li>Urobíš to <strong>kliknutím na ikonku bublinky</strong> so správou pri rozbalení kontaktu. Zobrazí sa PRIAMY CHAT, ktorý je však plne dostupný až po potvrdení handshake z druhej strany.</li>
          <li><strong>Žiadosť o kontakt</strong> môžeš odoslať so sprievodnou správou alebo bez nej. Partner na druhej strane môže handshake potvrdiť alebo odmietnuť.</li>
          ...
          <li><strong>Dôležité upozornenie:</strong> Ak si užívateľ neuložil žiadne kontaktné údaje, nezobrazia sa žiadne. <strong>LARIA nič nekontroluje.</strong> Tiež si dopredu premysli, koho žiadaš o kontakt: ak si totiž užívateľ neuložil žiadne údaje a ty svoje áno, on tvoje kontaktné údaje dostane a nie je možné ich spätne odstrániť.</li>
        </ul>
        <p>Staré a nezaujímavé kontakty je možné kedykoľvek odstrániť zo zoznamu jednoduchým kliknutím na ikonku koša v rozbalenom kontakte.</p>
      </div>
    `
  },
  {
    ...
    content: `
      <div class="faq-article-content">
        ...
        <p>Tento kód je potrebné vložiť v sekcii <em>Nastavenia</em> do políčka pre obnovenie účtu. Po vložení sa aktivuje tlačidlo „Obnoviť účet“. Po kliknutí je nutné počkať približne 5 sekúnd, kým LARIA preverí identitu.</p>
        <br/>
        ...
        <p>Netreba však vešať hlavu :-) Pri každom úspešnom handshake dostanú obidve strany na svoj LARIA účet malé poďakovanie v podobe <strong>0.0000 0001 LARIA</strong>.</p>
        <br/>
...
// --- Globálna funkcia pre zväčšenie obrázka (Modal / Lightbox) ---
if (typeof window !== "undefined") {
  window.openImageModal = function (src) {
    const existingModal = document.getElementById("laria-image-modal");
    if (existingModal) existingModal.remove();

    const modal = document.createElement("div");
    modal.id = "laria-image-modal";
    modal.className = "laria-modal-overlay";

    const img = document.createElement("img");
    img.src = src;
    img.className = "laria-modal-img";

    const closeBtn = document.createElement("span");
    closeBtn.innerHTML = "&times;";
    closeBtn.className = "laria-modal-close";
